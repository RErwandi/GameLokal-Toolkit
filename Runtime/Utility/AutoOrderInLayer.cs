...
namespace GameLokal.Toolkit
{
    ...
    public class AutoOrderInLayer : MonoBehaviour 
	{
		...
		[Tooltip("Add this component to an object with a sprite renderer, and it'll give it a new order in layer based on the settings defined here. First is the global counter increment, or how much you'd like to increment the layer order between two objects on that same layer.")]
		/// the number by which to increment each new object's order in layer
		public int GlobalCounterIncrement = 5;

		[Header("Parent")]
		[Tooltip("You can also decide to determine the new layer order based on the parent sprite's order (it'll have to be on the same layer).")]
		...
		[Header("Children")]
		[Tooltip("And here you can decide to apply your new layer order to all children.")]
